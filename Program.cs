...
try
{
    ...
    while (true)
    {
        ...
        // offsets.cs
        int dwEntityList = 0x1D15F88;
        int dwViewMatrix = 0x1E330F0;
        int dwLocalPlayerPawn = 0x1BF2490;
        ...
        while (true)
        {
            ...
            try
            {
                ...
                for (int ii = 0; ii < indices.Length; ii++)
                {
                    ...
                    // check if lifestate   
                    int lifeState = swed.ReadInt(currentPawn, m_lifeState) & 0xFF;
                    int health = swed.ReadInt(currentPawn, m_iHealth);
                    if (lifeState != 0 || health <= 0) continue;
                    ...
                    try
                    {
                        ...
                        // fix
                        byte[] nameBytes = swed.ReadBytes(currentController, m_iszPlayerName, 16);
                        entity.name = System.Text.Encoding.UTF8.GetString(nameBytes).TrimEnd('\0');
                    }
                    catch
                    {
                        continue; // if pizda
